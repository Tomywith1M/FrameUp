...
import 'package:frameup/state/demo_users.dart';
...
class __UserProfileState extends State<_UserProfile> {
  ...
  late Future<FrameUser> userDataFuture = getUser();

  Future<FrameUser> getUser() async {
    ...
    return FrameUser.fromMap(streamUser.data!);
  }
  ...
  @override
  Widget build(BuildContext context) {
    return FutureBuilder<FrameUser>(
    ...
    return Row(
      children: [
        Padding(
          ...
          child: Character.medium(frameUser: widget.userData),
        ),
        Padding(
          padding: const EdgeInsets.all(8.0),
          child: Column(
