...
import 'package:frameup/components/Opinions/state/opinion_state.dart';
import 'package:frameup/components/Widg/Characters.dart';
import 'package:frameup/components/Widg/dimmer.dart';
import 'package:frameup/components/Widg/favorite_icon.dart';
...
class OpinionScreen extends StatefulWidget {
  ...
  static Route route({
    ...
    required FrameUser activityOwnerData,
  }) =>
      MaterialPageRoute(
...
class _OpinionScreenState extends State<OpinionScreen> {
  late FocusNode opinionFocusNode;
  late OpinionState opinionState;

  @override
  void initState() {
    super.initState();
    opinionFocusNode = FocusNode();
  ...
  Widget build(BuildContext context) {
    return MultiProvider(
      providers: [
        ChangeNotifierProvider.value(value: opinionState),
        ChangeNotifierProvider.value(value: opinionFocusNode),
      ],
      child: GestureDetector(
        onTap: () {
          opinionState.resetCommentFocus();
...
class __OpinionBoxState extends State<_OpionionBox> {
  ...
  Future<void> handleSubmit(String? value) async {
    if (value != null && value.isNotEmpty) {
      ...
      final opinionState = context.read<OpinionState>();
      final commentFocus = opinionState.commentFocus;

      if (commentFocus.opinion == Opinion.activityOpinion) {
      ...
      } else if (commentFocus.opinion == Opinion.reactionOpinion) {
  ...
  Widget build(BuildContext context) {
    final commentFocus =
        context.select((OpinionState state) => state.commentFocus);
        ...
        child: Column(
          ...
          children: [
            AnimatedSwitcher(
              ...
              child: (commentFocus.opinion == Opinion.reactionOpinion)
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            ...
            Dimmer(
              onTap: () {
                context.read<OpinionState>().resetCommentFocus();
              },
              icon: Icons.close,
              size: 16,
...
class __CommentTileState extends State<_CommentTile> {
  late final userData = FrameUser.fromMap(widget.reaction.user!.data!);
  ...
  late bool isfavorite = _isFavorited();
  Reaction? starReaction;

  String _timeSinceComment() {
    final jiffyTime = Jiffy(widget.reaction.createdAt).fromNow();
    if (jiffyTime == 'a couple of seconds ago') {
      return 'About Now';
    } else {
      return jiffyTime;
    }
  ...
  bool _isFavorited() {
    starReaction = widget.reaction.ownChildren?['like']?.first;
    return starReaction != null;
  }

  Future<void> _handleFavorite(bool liked) async {
    if (isfavorite && starReaction != null) {
      await context.appState.client.reactions.delete(starReaction!.id!);
      numberOfLikes--;
    } else {
      starReaction = await context.appState.client.reactions.addChild(
        'favorite',
        widget.reaction.id!,
          ...
          children: [
            Padding(
              ...
              child: (widget.isReplyToComment)
                  ? Character.tiny(frameUser: userData)
                  : Character.small(frameUser: userData),
            ),
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Visibility(
                          ...
                          child: GestureDetector(
                            onTap: () {
                              context.read<OpinionState>().setCommentFocus(
                                    CommentFocus(
                                      opinion: Opinion.reactionOpinion,
                                      id: widget.reaction.id!,
                                      user: FrameUser.fromMap(
